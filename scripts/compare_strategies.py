...
def print_comparison():
    ...
    if wr_diff > 0:
        ...
        print(f"⚠️  Intraday strategy has {abs(wr_diff):.1%} points LOWER win rate")
    ...
    if pf_diff > 0:
        ...
        print(f"⚠️  Intraday strategy has {abs(pf_diff):.2f}x WORSE profit factor")
    ...
    if intraday_roi > eod_roi and pnl_diff > 0:
        ...
        if wr_diff < 0:
            print("   ⚠️  Trade-off: Lower win rate (but better average wins)")
        ...
        if intraday_roi > eod_roi:
            print("   ℹ️  Note: Intraday has better ROI but EOD more reliable")
    else:
        print("🤝 MIXED RESULTS")
